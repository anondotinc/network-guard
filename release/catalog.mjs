...
export const R2_DOWNLOAD_ORIGIN = 'https://downloads.anon.inc';
// Previously signed catalogs keep their exact URLs; new imports use the domain above.
export const R2_DOWNLOAD_ORIGINS = Object.freeze([R2_DOWNLOAD_ORIGIN, 'https://release.anon.inc']);
...
export function validateCatalog(catalog) {
  ...
  catalog.releases.forEach((r, i) => {
    ...
    r.artifacts.forEach((a, j) => {
      ...
      if (!isR2 && !(r.product === 'network-helper' && a.location === helperArtifactUrl(r, a.filename))) fail(ap, 'location must match the versioned R2 path or official helper GitHub release asset');
...
/** Preserve release history and withdrawals when importing a newer signed catalog. */
export function assertCatalogTransition(previous, next) {
  ...
  for (const before of previous.releases) {
    ...
    require(after, 'Catalog cannot silently remove release history; mark the release withdrawn');
    require(JSON.stringify(before.artifacts) === JSON.stringify(after.artifacts) && before.releasedAt === after.releasedAt && before.channel === after.channel, 'A versioned release cannot change artifact bytes, identity, date, or channel');
    require(before.status !== 'withdrawn' || after.status === 'withdrawn', 'Withdrawn versions cannot be reactivated; create a new version/build');
    require(before.status !== 'published' || after.status !== 'unreleased', 'Published versions cannot return to unreleased');
  }
}
