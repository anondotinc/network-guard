...
`downloads.anon.inc` for APKs and shared catalog metadata; new helper assets use
...
- Provision a **new dedicated release bucket** and its `downloads.anon.inc`
...
node release/cli.mjs package-helper \
  ...
  --metadata-private-key /absolute/protected/metadata-private-key.pem \
  --execute-signing --out release/out/selected-release
```

The version and build ID must match the app's `CFBundleShortVersionString` and
`CFBundleVersion`. The example matches the current build-4 source. To create a
new release identity, update the source version/build and rebuild; the packager
refuses to relabel an existing app with a different build number.

Metadata signing is part of this explicit pipeline. Before contacting Apple,
the private key must match `release/trust/anon-release-metadata.pub` (override
with `--metadata-public-key` only for a separately reviewed publisher/trust
configuration). After notarization, stapling and final hashing, `metadata/`
contains the inactive signed catalog, raw detached signature, checksums and public
key. This does not activate a download. Ordinary source builds need no secrets.
...
4. Download every non-withdrawn artifact again through its exact signed
   `https://downloads.anon.inc/<product>/<version>/<build>/<filename>` URL.
