...
| `sign` | Candidate + artifacts + external Ed25519 private key matching the pinned public key | None; exact-byte detached metadata signing |
...
The official release key is pinned in [the public key](trust/anon-release-metadata.pub)
and [discovery record](trust/anon-releases.json). It is dedicated to Android and
Network Guard release metadata, not Apple/Android package signing, wallet keys,
or the paymaster. The same records are published under Anon's `.well-known`.
Read [the verification guide](https://docs.anon.inc/security/release-verification).
Private keys remain outside all repositories with owner-only permissions.
Back up the private key in approved encrypted offline storage before public
rollout; losing it prevents future signatures under this trust identity.
...
release. Key rotation requires an independently announced trust-pin update;
never trust a replacement key solely from its own signed catalog. Do not generate
or commit a production signing key here. `sign` refuses
private keys inside any Git checkout or readable by other users, and refuses a
key that differs from the independently pinned public key. Signed helper
packaging now performs the same preflight before Apple submission and produces
signed inactive metadata after final packaging.
