...
import { assertCatalog, R2_DOWNLOAD_ORIGIN } from './catalog.mjs';
...
export async function importApk({ apk, signer, version, buildId, releasedAt, channel = 'stable', out, aapt = 'aapt2', apksigner = 'apksigner', notes = [] }, execute = run) {
  ...
  const artifact = { platform: 'android', architecture: metadata.architecture, minimumOs: metadata.minimumOs, filename, ...after, signing: { kind: 'android', certificateSha256: metadata.certificateSha256, packageName: metadata.packageName }, location: `${R2_DOWNLOAD_ORIGIN}/android/${version}/${buildId}/${filename}` };
